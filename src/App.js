...
class App extends Component {
  ...
  onClickThumbnail = id => {
    const {currentImagesId} = this.state
  ...
  generateRandomImage = () => {
    ...
    const currentImagesId = imagesList[index].id
    const {score} = this.state
    this.setState({currentImagesId, score: score + 1})
  }
  ...
  render() {
    const {activeTabId, currentImagesId, score, timer, gameOver} = this.state
    const thumbnailImagesList = imagesList.filter(
      imagesDetails => imagesDetails.category === activeTabId,
    )
        ...
        <div className="game-view-container">
          {gameOver ? (
            ...
            <Game
              ...
              currentImagesId={currentImagesId}
